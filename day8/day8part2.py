...
class Processor:
    ...
    def run(self):
        """
        Run the program. Returns True if it gets to the last statement, otherwise False
        """
        try:
...
def main():
    ...
    # Try running the program and switching one JMP to a NOP or NOP to a JMP
    for i,v in enumerate(data):
        if v.opcode == OpCode.jmp or v.opcode == OpCode.nop:
            # switch operation
            saved_opcode = v.opcode
            if v.opcode == OpCode.jmp:
                v.opcode = OpCode.nop  
            elif v.opcode == OpCode.nop:
                v.opcode = OpCode.jmp  

            processor = Processor(data)
            # add in a processor observer to detect running same instruction
            detector = DetectInfiniteLoop(processor)
            if processor.run() == True:
                print(f"Result: Change Istruction at {i} to {v}. ACC={processor.ACC}")
                return

            v.opcode = saved_opcode
